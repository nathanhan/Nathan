...
/*
  ...
  */
  bool areSecondaryAlignsKept = true; //ie yes I should keep shittier matches of search
  ...
  while (contigBamParser.GetNextRead(contig,isReadValid)) {
    ...
    if (isReadValid) {
      ...
      for (int i = 0; i < clippedContigSeqs.size(); i++) {
      	...
      	cout << virusesSearchResultTempContainer[0] << endl;
      	/*//search on bacteria
